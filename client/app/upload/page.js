...
export default function UploadPage() {
  ...
  const processSingleFile = async (file, token) => {
    ...
    try {
      ...
      const response = await fetch('https://legalai-xx7y.onrender.com/api/ocr/upload-single', {
        method: 'POST',
        headers: {
  ...
  const processMultipleFiles = async (files, token) => {
    ...
    const progressInterval = setInterval(() => {
      ...
      const response = await fetch('https://legalai-xx7y.onrender.com/api/ocr/upload-multiple', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
